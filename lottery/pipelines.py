...
def process_data(converted_data, offset):
    ...
    x_attributes = []
    y_attributes = []
    result[config.ATTRIBUTE_ISSUE] = converted_data[config.ATTRIBUTE_ISSUE].values[offset+1:]
    ...
    for ball_item in config.BALL_ENUM:
        ...
        result[Y_ATTRIBUTES[k]] = converted_data[Y_ATTRIBUTES[k]][offset+1:].values
        for column in X_ATTRIBUTES[k]:
            result[column] = converted_data[column][offset+1:].values
