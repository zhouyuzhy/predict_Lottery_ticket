...
STOCK_CODES = [STOCK_CODE_MEITUAN, STOCK_CODE_TENGXUN, STOCK_CODE_BABA, STOCK_CODE_MI, STOCK_CODE_HENGSHENG,
               STOCK_CODE_JD, STOCK_CODE_BYD, STOCK_CODE_JILI, STOCK_CODE_ZHONGXINGUOJI, STOCK_CODE_HENGSHENG_KEJI]
...
def set_rsi(data_pd):
    ...
    data_pd['crossover_6_12'] = [crossesOver(data_pd['rsi6'].values[:i + 1], data_pd['rsi12'].values[:i + 1]) for i in
                                 range(len(data_pd))]
    data_pd['crossover_6_24'] = [crossesOver(data_pd['rsi6'].values[:i + 1], data_pd['rsi24'].values[:i + 1]) for i in
                                 range(len(data_pd))]
    data_pd['crossover_12_24'] = [crossesOver(data_pd['rsi12'].values[:i + 1], data_pd['rsi24'].values[:i + 1]) for i in
                                  range(len(data_pd))]
    upper_rsi = 80
    lower_rsi = 30
    ...
    data_pd['crossunder_6_12'] = [crossesUnder(data_pd['rsi6'].values[:i + 1], data_pd['rsi12'].values[:i + 1]) for i in
                                  range(len(data_pd))]
    data_pd['crossunder_6_24'] = [crossesUnder(data_pd['rsi6'].values[:i + 1], data_pd['rsi24'].values[:i + 1]) for i in
                                  range(len(data_pd))]
    data_pd['crossunder_12_24'] = [crossesUnder(data_pd['rsi12'].values[:i + 1], data_pd['rsi24'].values[:i + 1]) for i
